# comp3-project
awesome multiplayer (8 player probably) puzzle platformer 

### Features

 - $(Game Feature 01)
 - $(Game Feature 02)
 - $(Game Feature 03)

### Controls

Keyboard:
 - $(Game Control 01)
 - $(Game Control 02)
 - $(Game Control 03)

### Screenshots

_TODO: Show your game to the world, animated GIFs recommended!._

## Compiling

1. Clone the code
    1. `git clone https://github.com/Milo-C-6/comp3-project.git`
2. Install w64devkit from [/skeeto/w64devkit/](https://github.com/skeeto/w64devkit/releases/latest) and make sure its on path
3. Setup cmake configuration
    1. `cmake -S . -B build`
4. Build
    1. `cmake --build build`

Executable file will be in  (`../build/${PROJECT_NAME}/${PROJECT_NAME}`)
Currently running it will just load the gameplay screen, however you can add `-e` to the end to launch the editor.

## Credits

 - [Milo](https://github.com/Milo-C-6)
 - [Trace](https://github.com/CoolGuyTrace)
 - [@raysan5](https://github.com/raysan5) - for making raylib & raygui <3

### Links

 - YouTube Gameplay: $(YouTube Link)
 - itch.io Release: $(itch.io Game Page)
 - Steam Release: $(Steam Game Page)